...
function findLongestWord(sentence) {
  ...
  const wordLengthArr = [];
  sentenceArr.map((word) => wordLengthArr.push(word.length));
  return sentenceArr.filter(
    (word) => word.length === Math.max(...wordLengthArr)
  )[0];
}
    ...
    .reduce((acc, char) => {
      if (char.charCodeAt(0) >= 65 && char.charCodeAt(0) <= 90) {
        if (char.charCodeAt(0) < 78) {
          acc.push(String.fromCharCode(char.charCodeAt(0) + 13));
        } else {
          acc.push(String.fromCharCode(char.charCodeAt(0) - 13));
        }
